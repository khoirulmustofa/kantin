                ...
                <div class="swiper-wrapper">
                    <div class="swiper-slide relative">
                        ...
                        <div
                            ...
                            <h2
                                class="text-3xl md:text-7xl font-bold text-white mb-2 md:mb-4 uppercase tracking-tighter">
                                New Arrivals</h2>
                            <p class="mb-4 text-white md:text-2xl font-medium">Experience the best in quality with
                                <br>our latest collection.
                            </p>
                            <Link :href="route('product.index')"
                                class="bg-blue-600 hover:bg-white text-white hover:text-blue-600 font-black px-10 py-4 rounded-2xl uppercase text-xs tracking-widest transition-all shadow-2xl shadow-blue-500/30">
                                Shop now
                            </Link>
                        </div>
                    </div>
                    ...
                    <div class="swiper-slide relative">
                        ...
                        <div
                            ...
                            <Link :href="route('product.index')"
                                class="bg-white hover:bg-blue-600 text-black hover:text-white font-black px-10 py-4 rounded-2xl uppercase text-xs tracking-widest transition-all">
                                Shop now
                            </Link>
                        </div>
                    </div>
        ...
        <section id="product-banners" class="bg-white">
            <div class="container mx-auto py-20 px-4">
                <div class="flex flex-col items-center mb-16">
                    <span class="text-blue-600 font-black text-xs uppercase tracking-[0.3em] mb-4">Curated
                        Selection</span>
                    <h2 class="text-4xl md:text-5xl font-black text-gray-900 tracking-tighter">Shop by Category</h2>
                </div>
                    ...
                    <div v-for="category in categories" :key="category.id" class="w-full sm:w-1/2 lg:w-1/4 px-4 mb-8">
                        ...
                        <Link :href="route('product.index', { category: category.slug })"
                            ...
                            <div
                                ...
                                <span
                                    class="text-[10px] font-bold uppercase tracking-[0.2em] mb-2 opacity-70 group-hover:opacity-100 transition-opacity">
                                    {{ category.products_count }} Products
                                </span>

                                <h3
                                    class="text-xl font-black mb-4 tracking-tighter uppercase group-hover:scale-105 transition-transform">
                                    {{ category.name }}
                                </h3>

                                <div
                                    class="w-10 h-1 bg-blue-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 rounded-full">
                                </div>
                            </div>
                        </Link>
                    </div>
            ...
            <div class="container mx-auto px-4">
                <div class="flex flex-col md:flex-row justify-between items-end mb-16 gap-4">
                    <div class="flex flex-col">
                        <span class="text-blue-600 font-black text-xs uppercase tracking-[0.3em] mb-4">Trending
                            Now</span>
                        <h2 class="text-4xl md:text-5xl font-black text-gray-900 tracking-tighter">Popular Selection
                        </h2>
                    </div>
                    <Link :href="route('product.index')"
                        class="text-xs font-black uppercase tracking-widest text-gray-400 hover:text-blue-600 transition-colors flex items-center gap-2">
                        View All Collections <i class="pi pi-arrow-right text-[10px]"></i>
                    </Link>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    <div v-for="product in products" :key="product.id"
                        class="group bg-white p-4 rounded-3xl border border-gray-100 hover:border-blue-100 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-200">
                        <div
                            ...
                            <div
                                ...
                                <Link :href="route('product.show', product.slug)"
                                    class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-black hover:bg-blue-600 hover:text-white transition-all transform translate-y-4 group-hover:translate-y-0 duration-500 shadow-xl">
                                    <i class="pi pi-eye text-lg"></i>
                                </Link>
                                <button @click="cartStore.addItem(product)" :disabled="product.stock === 0"
                                    class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-black hover:bg-blue-600 hover:text-white transition-all transform translate-y-4 group-hover:translate-y-0 duration-700 shadow-xl disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        <Link :href="route('product.show', product.slug)">
                            <h3
                                class="font-black text-gray-900 group-hover:text-blue-600 transition-colors text-lg tracking-tight mb-2 ">
                                {{ product.name }}</h3>
                            <div class="flex"> <span
                                    class="inline-flex items-center bg-blue-600/10 text-blue-600 text-[10px] font-bold px-2.5 py-0.5 rounded-full tracking-wider mb-4 border border-blue-600/20">
                                    {{ product.category?.name }}
                                </span>
                            </div>
                            <div class="text-2xl font-black flex justify-between items-center py-2 border-t border-gray-50">
