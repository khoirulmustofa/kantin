...
const loadLazyData = (extraParams = {}) => {
    router.get(route('admin.financial_accounts.index'), {
        search: search.value,
        rows: extraParams.rows || props.accounts.per_page,
...
<template>
    ...
    <AccountForm v-model:visible="accountDialog" :account="selectedAccount" :isEdit="isEdit" />
    ...
    <AdminLayout v-model:menuActive="props.menu" v-model:title="props.title">
        <div
            ...
            <Toolbar class="!border-0 !border-b !p-0 !pb-2 dark:!bg-gray-800">
                <template #start>
                    <div class="w-full sm:w-auto">
                        <Button label="New Account" icon="pi pi-plus" severity="primary" @click="openNew"
                ...
                <template #end>
                    <div class="w-full flex justify-end">
                        <IconField class="w-full">
                            ...
                            <InputText v-model="search" placeholder="Search accounts..." class="w-full sm:w-64" />
            ...
            <DataTable :value="accounts.data" lazy paginator sortMode="multiple" :multiSortMeta="multiSortMeta"
                :first="(accounts.current_page - 1) * accounts.per_page" :rows="accounts.per_page"
                :totalRecords="accounts.total" scrollable
                ...
                <template #loading> Loading account data... </template>
                ...
                <Column field="balance" header="Balance" sortable style="min-width: 12rem">
                    <template #body="slotProps">
                        <span class="font-bold">{{ formatCurrencyIndo(slotProps.data.balance) }}</span>
                    </template>
                </Column>
