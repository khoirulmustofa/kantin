...
import InflowForm from './Form.vue';
...
const loadLazyData = (extraParams = {}) => {
    router.get(route('admin.financial_cash_in.index'), {
        search: search.value,
...
const confirmDeleteAction = (data) => {
    confirm.require({
        message: `Are you sure you want to delete this inflow of ${formatCurrencyIndo(data.amount)}?`,
        header: 'Confirm Delete',
        icon: 'pi pi-exclamation-triangle',
        acceptClass: 'p-button-danger',
        accept: () => {
            loading.value = true;
            router.delete(route('admin.financial_cash_in.destroy', data.id), {
                onSuccess: () => {
                    toast.add({ severity: 'success', summary: 'Successful', detail: 'Inflow Deleted', life: 3000 });
                },
                onFinish: () => {
                    loading.value = false;
                }
...
<template>
    ...
    <InflowForm v-model:visible="inflowDialog" :mutation="selectedMutation" :isEdit="isEdit" :accounts="accounts"
    ...
    <AdminLayout v-model:menuActive="props.menu" v-model:title="props.title">
        <div
            ...
            <Toolbar class="!border-0 !border-b !p-0 !pb-2 dark:!bg-gray-800">
                <template #start>
                    <div class="w-full sm:w-auto">
                        <Button label="New Inflow" icon="pi pi-plus" severity="primary" @click="openNew"
                ...
                <template #end>
                    <div class="w-full flex justify-end">
                        <IconField class="w-full">
                            ...
                            <InputText v-model="search" placeholder="Search inflows..." class="w-full sm:w-64" />
            ...
            <DataTable :value="mutations.data" lazy paginator sortMode="multiple" :multiSortMeta="multiSortMeta"
                ...
                <template #empty> No cash inflows found. </template>
                <template #loading> Loading inflow data... </template>
                ...
                <Column field="category.name" header="Category" sortable style="min-width: 12rem">
                    <template #body="slotProps">
                        <Tag :value="slotProps.data.category?.name" severity="success" rounded
