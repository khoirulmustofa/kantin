...
const saveOutflow = () => {
    if (props.isEdit) {
        form.transform((data) => ({
            ...data,
            _method: 'PUT',
        })).post(route('admin.financial_cash_out.update', props.mutation.id), {
            forceFormData: true,
            onSuccess: () => {
                hideDialog();
                toast.add({ severity: 'success', summary: 'Successful', detail: 'Outflow Updated', life: 3000 });
            },
        });
    } else {
        form.post(route('admin.financial_cash_out.store'), {
            forceFormData: true,
            onSuccess: () => {
                hideDialog();
                toast.add({ severity: 'success', summary: 'Successful', detail: 'Outflow Recorded', life: 3000 });
            },
        });
    }
};
...
<template>
    <Dialog v-model:visible="visible" :style="{ width: '500px' }" :header="isEdit ? 'Edit Outflow' : 'New Cash Outflow'"
        ...
        <div class="flex flex-col gap-5 mt-2">
            ...
            <div class="flex flex-col gap-2">
                <label for="account" class="text-sm font-bold text-gray-700 dark:text-gray-300">Source Account</label>
            ...
            <div class="flex flex-col gap-2">
                <label for="category" class="text-sm font-bold text-gray-700 dark:text-gray-300">Expense
                    Category</label>
        ...
        <template #footer>
            <div class="flex justify-end gap-2 mt-6">
                ...
                <Button type="button" label="Record Outflow" @click="saveOutflow" :loading="form.processing"
                    class="px-8 shadow-lg shadow-rose-500/20 bg-rose-600 border-rose-600 hover:bg-rose-700" />
            </div>
        </template>
    </Dialog>
</template>
